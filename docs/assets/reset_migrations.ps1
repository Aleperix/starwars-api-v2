...
try {
    ...
    if ($dbManager -eq "postgresql") {
        psql -U $dbUser -W -d $dbName -f "$scriptDirectory/starwars_psql_data.sql"; if ($?) { Write-Output "DATA POPULATED" }
        if ($LASTEXITCODE -ne 0) { throw "Failed to populate data" }
    } else {
        Get-Content "$scriptDirectory/starwars_mysql_data.sql" | mysql -h $dbUrl -u $dbUser -p $dbName; if ($?) { Write-Output "DATA POPULATED" }
        if ($LASTEXITCODE -ne 0) { throw "Failed to populate data" }
    }
